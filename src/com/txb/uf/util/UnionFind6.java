...
/**
 * 并查集 UnionFind5  的优化，进行路径压缩
 * 把路径深度降低成2，不管是哪种链表，最后都直接指向根节点，也就是说这个成为了n 叉树
 */
public class UnionFind6 implements UF {
    ...
    private int find(int p) {
        if (p != parent[p]) {//不是根节点，
            //find(parent[p]) 返回父节点对应的根节点
            //p的父节点等于这个父节点对应的根节点，
            parent[p] = find(parent[p]);//p
        }
        return parent[p];//返回p的根节点，也就是整个树的根节点
    }
